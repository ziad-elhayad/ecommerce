...
export default function CheckoutPage() {
  ...
  const handleOrder = async (orderType: 'cash' | 'online') => {
    ...
    if (!currentCartId) {
      setLoading(false);
      if (items.length === 0) setError('the cart is empty , please add a products ');
      else setError(`fail to load products. (${debugError || 'Unknown Error'})`);
      return;
    }
    ...
    try {
      ...
      if (err.response?.status === 401) { setError('error, try again later '); return; }
      setError(err.message || 'fail in create an account');
